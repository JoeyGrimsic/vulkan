...
// Pass last and current SSBO to the shaders, see my comp shader chapter for the Vulkan tutorialcomp
...
public:
  ...
  VulkanExample() : VulkanExampleBase() { title = "Compute shader particle system"; }
  ...
  void loadAssets() {
    textures.particle.loadFromFile(getAssetPath() + "textures/particle01_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice,
                                   queue);
    textures.gradient.loadFromFile(getAssetPath() + "textures/particle_gradient_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice,
                                   queue);
  }
  ...
  void prepareStorageBuffers() {
    ...
    for (auto &storageBuffer : storageBuffers) {
      ...
      if (graphics.queueFamilyIndex != compute.queueFamilyIndex) {
        ...
        vkCmdPipelineBarrier(copyCmd, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 0, nullptr, 1,
  ...
  void setupDescriptorPool() {
    ...
    VkDescriptorPoolCreateInfo descriptorPoolInfo =
        vks::initializers::descriptorPoolCreateInfo(poolSizes, maxConcurrentFrames * 2);
  ...
  void prepareGraphics() {
    ...
    writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(
        graphics.descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &textures.particle.descriptor));
    // Binding 1 : Particle gradient ramp
    writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(
        graphics.descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &textures.gradient.descriptor));
  ...
  void prepareCompute() {
    ...
    // The VulkanDevice::createLogicalDevice functions finds a compute capable queue and prefers queue families that only support
    // compute Depending on the implementation this may result in different queue family indices for graphics and computes,
    ...
    VkCommandBufferAllocateInfo cmdBufAllocateInfo =
        vks::initializers::commandBufferAllocateInfo(compute.commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, 1);
    ...
    // We need the previous particle positions to calculate data for the current frame
    std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
    ...
    for (auto i = 0; i < compute.uniformBuffers.size(); i++) {
      ...
      std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets = {
          // Binding 0 : Previous particles storage buffer
          vks::initializers::writeDescriptorSet(compute.descriptorSets[i], VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 0,
                                                &storageBuffers[(i - 1) % maxConcurrentFrames].descriptor),
          // Binding 1 : Current particles storage buffer
          vks::initializers::writeDescriptorSet(compute.descriptorSets[i], VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                                                &storageBuffers[i].descriptor),
          // Binding 2 : Uniform buffer
          vks::initializers::writeDescriptorSet(compute.descriptorSets[i], VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 2,
                                                &compute.uniformBuffers[i].descriptor)};
      vkUpdateDescriptorSets(device, static_cast<uint32_t>(computeWriteDescriptorSets.size()), computeWriteDescriptorSets.data(),
                             0, nullptr);
    }
    ...
    computePipelineCreateInfo.stage =
        loadShader(getShadersPath() + "computeparticles/particle.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
  ...
  void buildGraphicsCommandBuffer() {
    ...
    if (graphics.queueFamilyIndex != compute.queueFamilyIndex) {
      ...
      vkCmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT, 0, 0, nullptr, 1,
                           &buffer_barrier, 0, nullptr);
    }
    ...
    vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphics.pipelineLayout, 0, 1, &graphics.descriptorSet, 0,
                            nullptr);
    ...
    if (graphics.queueFamilyIndex != compute.queueFamilyIndex) {
      ...
      vkCmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 0, nullptr, 1,
                           &buffer_barrier, 0, nullptr);
    }
  ...
  void buildComputeCommandBuffer() {
    ...
    // Add memory barrier to ensure that the (graphics) vertex shader has fetched attributes before compute starts to write to the
    // buffer
    if (graphics.queueFamilyIndex != compute.queueFamilyIndex) {
      VkBufferMemoryBarrier buffer_barrier = {VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
                                              nullptr,
      ...
      vkCmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, nullptr, 1,
                           &buffer_barrier, 0, nullptr);
    }

    // Dispatch the compute job
    vkCmdBindPipeline(cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute.pipeline);
    vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute.pipelineLayout, 0, 1,
                            &compute.descriptorSets[currentBuffer], 0, 0);
    ...
    if (graphics.queueFamilyIndex != compute.queueFamilyIndex) {
      ...
      vkCmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 0, nullptr,
