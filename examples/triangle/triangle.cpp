 ...
 *
 * Note:
 *	This is a "pedal to the metal" example to show off how to get Vulkan up
 * and displaying something Contrary to the other examples, this one won't make
 * use of helper functions or initializers Except in a few cases (swap chain
 * setup e.g.)
 *
...
// We want to keep GPU and CPU busy. To do that we may start building a new
// command buffer while the previous one is still being executed This number
// defines how many frames may be worked on simultaneously at once Increasing
// this number may improve performance but will also introduce additional
// latency
...
public:
  ...
  struct {
    VkDeviceMemory memory{
        VK_NULL_HANDLE}; // Handle to the device memory for this buffer
    VkBuffer buffer{VK_NULL_HANDLE}; // Handle to the Vulkan buffer object that
                                     // the memory is bound to
  } vertices;
  ...
  // Create the per-frame (in flight) Vulkan synchronization primitives used in
  // this example
  ...
  void createVertexBuffer() {
    ...
    //
    // Note: On unified memory architectures where host (CPU) and GPU share the
    // same memory, staging is not necessary To keep this sample easy to follow,
    // there is no check for that in place
  ...
  // many and what types of descriptors we are going to use (at maximum)
  ...
  // shader Basically connects the different shader stages to descriptors for
  // binding uniform buffers, image samplers, etc. So every shader binding
  // should map to one descriptor set layout binding
  ...
  // Shaders access data using descriptor sets that "point" at our uniform
  // buffers The descriptor sets make use of the descriptor set layouts created
  // above
  void createDescriptorSets() {
    ...
    for (uint32_t i = 0; i < MAX_CONCURRENT_FRAMES; i++) {
      ...
      VK_CHECK_RESULT(vkAllocateDescriptorSets(
          device, &allocInfo, &uniformBuffers[i].descriptorSet));
  ...
  void createUniformBuffers() {
    // Prepare and initialize the per-frame uniform buffer blocks containing
    // shader uniforms Single uniforms like in OpenGL are no longer present in
    // Vulkan. All hader uniforms are passed via uniform buffer blocks
    VkMemoryRequirements memReqs;
    ...
    for (uint32_t i = 0; i < MAX_CONCURRENT_FRAMES; i++) {
      ...
      // correct one to allocate memory from is crucial We also want the buffer
      // to be host coherent so we don't have to flush (or sync after every
      // update. Note: This may affect performance so you might not want to do
      // this in a real world application that updates buffers on a regular base
      ...
      VK_CHECK_RESULT(vkBindBufferMemory(device, uniformBuffers[i].buffer,
                                         uniformBuffers[i].memory, 0));
      // We map the buffer once, so we can update it without having to map it
      // again
      VK_CHECK_RESULT(vkMapMemory(device, uniformBuffers[i].memory, 0,
                                  sizeof(ShaderData), 0,
  ...
  void render() override {
    ...
    // so we must use the acquire function and can't just cycle through the
    ...
    // Unlike in OpenGL all rendering commands are recorded into command buffers
    // that are then submitted to the queue This allows to generate work upfront
    // in a separate thread For basic command buffers (like in this sample),
    // recording is so fast that there is no need to offload this
    ...
    // Bind descriptor set for the current frame's uniform buffer, so the shader
    // uses the data from that buffer for this draw
    ...
    submitInfo.pCommandBuffers =
        &commandBuffer; // Command buffers(s) to execute in this batch
                        // (submission)
    submitInfo.commandBufferCount = 1; // We submit a single command buffer

    // Semaphore to wait upon before the submitted command buffer starts
    // executing
