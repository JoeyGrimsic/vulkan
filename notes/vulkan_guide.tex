...
\par I am considering making a dictionary because there are a lot of
terms for me to learn so it would be nice to have a spot I can
reference in the future to remember what something is.

\textbf{SPIR-V}: GLSL, HLSL, and Slang can all be compiled to SPIR-V,
which is passed to Vulkan program \\
\textbf{Slang}: A shading language that benefits from having generics
and interfaces that glsl doesn't have\\
\textbf{Semaphore}: Object responsible for keeping different GPU cores synced\\
\textbf{Fence}: Object responsible for keeping CPU and GPU synced\\
\textbf{Virtual Function}: A C++ feature relating to polymorphism and
inheritance of classes\\
\textbf{Sampling}: Query a subpixel value\\
\textbf{Aliasing}: When \textbf{jaggies} misrepresent a pixel and
cause discoloring\\
\textbf{Anti Aliasing}: technique to combat \textbf{aliasing},
usually by some special way of sampling\\
\textbf{Multisampling Antialiasing (MSAA)}: An implementation of
\textbf{anti aliasing}\\
\textbf{Super Sampling Antialiasing (SSAA)}: An implementation of
\textbf{anti aliasing}\\
\textbf{Synchronization2}: idk\\
\textbf{Dynamic Rendering}: This is the idea of removing the 'render
pass' and 'framebuffer' objects. In other words, we move the
configuration from initialization to recording time\\
\textbf{Max Concurrent Frames}: Number of frames to be 'processed'
simultaneously\\
\textbf{Memory Barriers}: Used to make sure that one GPU core is
finished writing to memory before another core tries to read\\
\textbf{Depth Image}: idk\\
\textbf{Z Buffer/ Depth Buffer}: idk\\
\textbf{Attachment}: idk, i think they are apart of frame buffers?\\
\textbf{Vk Queue Present KHR}: The command that tells the OS to
display the final image\\
\textbf{Device Idle}: Command to pause the CPU until the GPU is finished\\
\textbf{Application Object}: idk\\
\textbf{VRAM}: Stores memory for GPU\\
\textbf{Device-Local}: Memory on the VRAM\\
\textbf{Swap Chain}: A queue of rendered images waiting to be displayed\\
\textbf{Viewport}: idk\\
\textbf{In-Flight Frame}: Idea of rendering current frame on GPU, and
recording commands for next frame to be rendered, using CPU\\
\textbf{Rasterization}: Part in graphics pipeline when we draw?\\
\textbf{Render}: The final output of the graphics pipeline?\\
\textbf{Attributes}: I think attributes are when we store data in vertices\\
\textbf{Command Pools}: idk, a way to allocate commands?\\
\textbf{Ticket Number}: A timeline semaphore tracks completed tasks
in strictly increasing number/sequence. A ticket number (a positive
integer) is a certain complete task that we might wait for until we
do something else\\
\textbf{Binary Semaphore (Mutex)}: Has two states(0 or 1), used to
lock a resource, and managed by an owner\\
\textbf{Counting Semaphore}: Allows up to $N$ threads to access a
resource simultanously\\
\textbf{Timeline Semaphore}: Normal semaphore tracks complete or not
complete\textemdash a \textbf{timeline semaphore} tracks more than 1
complete steps in increasing order\\
\textbf{Subresource}: A part of an image\\
\textbf{Descriptor Set}: idk\\
\textbf{Descriptor Pool}: idk\\
\textbf{Device}: Like the object that keeps track of the rendering
state of the Vulkan program?\\
\textbf{Instanced Mesh Rendering}: idk\\
\textbf{Point Rendering}: idk\\
\textbf{Queue Family}: idk\\
\textbf{vkRenderingAttachmentInfoKHR}: idk\\
\textbf{Queue Family Index}: idk\\
\textbf{Color Attachment}: idk\\
\textbf{Stencil Attachment}: idk\\
\textbf{Render Target Views}: idk\\
\textbf{Resource Barriers}: idk\\
\textbf{Mip}: idk\\
\textbf{Mip Map}: idk\\
...
{
  name={Command Pools},
  description={idk, a way to allocate commands?}
}
